...
program
  .version("1.0.0")
  .option("--init-tsconfig", "Init tsconfig.json", false)
  .option(
  ...
  .option("--single-quote", "Use single quotes instead of double quotes.", true)
    ...
    if (program.initTsconfig) {
      try {
        const tsconfigFile = fs.readFileSync(
          path.join(__dirname, "./tsconfig_practices.json"),
          "UTF-8"
        );
