  ...
  expressions.forEach((expression: ts.Expression) => {
    if (expression.getText().indexOf("this.props") > -1) {
      expression.getChildren().forEach((item: any) => {
        ...
        if (
          itemText.indexOf("this.props") < 0 &&
          itemText.indexOf("=") < 0 &&
          itemText.indexOf("this") < 0 &&
            itemText.indexOf("prop") < 0 &&
            itemText.indexOf(".") < 0
        ) {
...
function getStateTypeOfReactComponentClass(
  ...
  const initialState = getInitialStateFromClassDeclaration(
    classDeclaration,
    typeChecker
  );
  const initialStateIsVoid = initialState.kind === ts.SyntaxKind.VoidKeyword;
  const collectedStateTypes = getStateLookingForSetStateCalls(
    classDeclaration,
    typeChecker
  );
