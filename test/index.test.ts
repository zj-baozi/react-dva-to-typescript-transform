...
const transformToFolderMap: [string, TransformFactoryFactory[]][] = [
  [
    "class-instance-variables-transform",
    [classInstanceVariablesTransformFactoryFactory]
  ],
  [
...
const isJestUpdateSnapshotEnabled = !!_.intersection(process.argv, [
  "-u",
  "--updateSnapshot"
]).length;
...
for (const [testFolderName, getFactory] of transformToFolderMap) {
  describe(testFolderName.replace(/\-/g, " ").replace("transform", ""), () => {
    for (const folderName of fs.readdirSync(
      path.join(__dirname, testFolderName)
    )) {
      const folder = path.join(
        path.join(__dirname, testFolderName, folderName)
      );
